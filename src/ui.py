...
def _init_state():
    if "pipeline" not in st.session_state:
        pipeline = RAGPipeline(uploads_dir="uploads", vectorstore_dir="vectorstore")
        pipeline.load()
        st.session_state["pipeline"] = pipeline
    if "chat_history" not in st.session_state:
        st.session_state["chat_history"] = []
    if "llm_model" not in st.session_state:
        st.session_state["llm_model"] = "qwen2.5:7b"
...
def _render_sidebar():
    ...
    with st.sidebar:
        ...
        defaults = ["qwen2.5:7b","qwen2.5:3b","qwen2.5:14b","llama3.2:3b","mistral:7b"]
        ...
        top_k = st.slider("chunks", 1, 10, st.session_state["top_k"], label_visibility="collapsed")
        ...
        if file_list:
            ...
            for finfo in file_list:
                ...
                with col1:
                    ...
                    st.markdown(
                        f'<div class="file-crt">'
                        f'<span class="file-crt-name">{finfo["name"]}</span>'
            ...
            if sources:
                with st.expander(f"[SRC] {len(sources)} REFERENCE CHUNK(S)", expanded=False):
                    for i, src in enumerate(sources, 1):
                        page = f" | PG.{src['page_number']}" if src.get("page_number") else ""
                        text_preview = src['text'][:400] + ("..." if len(src['text']) > 400 else "")
                        st.markdown(
                            f'<div class="source-crt">'
                            f'<div class="source-crt-header">REF {i}: {src["source_file"]}{page}</div>'
                            f'{text_preview}'
                            f'</div>',
                            unsafe_allow_html=True,
                        )
